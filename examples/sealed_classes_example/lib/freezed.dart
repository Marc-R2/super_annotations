    ...
    final mc = Mixin(
      (m) => m
        ...
        ..methods.addAll([
          mapMethod.rebuild(
            (m) => m.body =
                refer('UnimplementedError').newInstance([]).thrown.code,
          ),
        ]),
    );
    output.body.add(mc);

    for (final factory in target.constructors.where((c) => c.factory)) {
        ...
        final fc = Class(
          (fc) => fc
              ...
              Constructor(
                (c) => c
                  ...
                  ..requiredParameters.addAll(
                    factory.requiredParameters.map(
                      (p) => p.rebuild(
                        (pp) => pp
                          ..toThis = true
                          ..type = null,
                  ...
                  ..optionalParameters.addAll(
                    factory.optionalParameters.map(
                      (p) => p.rebuild(
                        (pp) => pp
                          ..toThis = true
                          ..type = null,
            ...
            ..methods.addAll([
              mapMethod.rebuild(
                (m) => m.body = refer(
                  factory.name ?? factory.redirect!.symbol!.toLowerCase(),
